...
public class OBJWriter {
    ...
    public static void writeToZip(File zipFile, float[] vertices, short[] indices, int vertexSize, boolean flipTexCoordV) throws IOException {
        ...
        writeObjToOutputStream(objOutputStream, vertices, indices, vertexSize, name.substring(0, index > 0 ? index : name.length()));
    ...
    public static void writeToFiles(File objFile, File mtlFile, File textureFile, float[] vertices, short[] indices, int vertexSize, boolean flipTexCoordV) throws IOException {

        final String name = mtlFile.getName();
        final int index = name.lastIndexOf('.');
        writeObjToOutputStream(new FileOutputStream(objFile), vertices, indices, vertexSize, name.substring(0, index > 0 ? index : name.length()));
    ...
    private static void writeObjToOutputStream(OutputStream outputStream, float[] vertices, short[] indices, int vertexSize, String mtlfilename) throws IOException {
        ...
        for (int i = 0; i < vertices.length; i += vertexSize) {
            writePosition(writer, vertices[i], vertices[i + 1], vertices[i + 2]);
        }

        for (int i = 0; i < vertices.length; i += vertexSize) {
            writeNormal(writer, vertices[i + 3], vertices[i + 4], vertices[i + 5]);
        }
    ...
    private static void drawVertexColorsToCanvas(Canvas canvas, float[] vertices, short[] indices, int vertexSize, boolean flipV) {
        ...
        for (int i = 0; i < vertices2D.length; i += 2) {
            ...
            Color.abgr8888ToColor(c, vertices[iv + 8]);
